...
public class BookController {
	...
	/*
	  책 수정 화면은 책 입력화면 + 책 상세 화면이다. 책입력 화면의 화면 형식을 그대로 따라가지만 데이터베이스에 저장된 데이터만
	  그려주면된다. 따라서 책 데이터는 상세 화면 서비스에서 가지고오고, 뷰는 책 입력 화면을 복사
	 */
	 ...
	 @RequestMapping(value = "/update", method = RequestMethod.GET)
		...
		public ModelAndView deletePost(@RequestParam Map<String, Object> map) {
		    ...
		    if (isDeleteSuccess) {
		    	//삭제가 성공했으면 상세 페이지 없으므로 리스트로 리다이렉트
